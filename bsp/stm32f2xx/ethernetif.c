...
/* Ethernet pins configuration : RTL8201BL
************************************************/
/************************************************
        ETH_MDIO -------------------------> PA2
        ETH_MDC --------------------------> PC1
...
*************************************************/
  ...
  GPIO_InitStructure.Pull = GPIO_NOPULL;         // --> PULLUP or NOPULL?
  ...
  GPIO_InitStructure.Pin =  GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_8 | GPIO_PIN_10 | GPIO_PIN_11 | GPIO_PIN_12 | GPIO_PIN_13;
...
static void low_level_init(struct netif *netif)
{
  ...
  osThreadDef(tEth_if, ethernetif_input, osPriorityRealtime, 0, INTERFACE_THREAD_STACK_SIZE);
  osThreadCreate (osThread(tEth_if), netif);

  /* Enable MAC and DMA transmission and reception */
  HAL_ETH_Start(&EthHandle);

#ifdef ETH_LINK_DETECT // RTL8201BL has not PHY_MICR and PHY_MISR
  uint32_t regvalue = 0;
  ...
  for( ;; )
  {
    if (osSemaphoreWait( s_xSemaphore, TIME_WAITING_FOR_INPUT)==osOK)
    {
      do
      {
		...
		//kprintf("ethernetif_input: p=0x%x len=%d\n", p, p->tot_len);
        if (p != NULL)
        {
