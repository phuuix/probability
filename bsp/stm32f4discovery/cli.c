...
int cmd_set_trace(struct shell_session *ss, int argc, char **argv)
{
	uint8_t block_id, level, level_ind;
	if(argc < 3)
	{
		ss->output("t <block_id> <trace_level>\n");
	}
	else
	{
		block_id = strtol(argv[1], NULL, 10);
		level_ind = strtol(argv[2], NULL, 10);
		for(level=0; level<8; level++)
		{
			if(level_ind & (1<<level))
			{
				uprintf_set_enable(level, block_id, 1);
				ss->output("enable block %d level %d\n", block_id, level);
			}
			else
				uprintf_set_enable(level, block_id, 0);
		}
	}
	return 0;
}
...
int cmd_process(char *cmd)
{
	...
	switch(*ch)
	{
		...
		case 't':
		{
			cmd_set_trace(&session, argc, argv);
			break;
		}
