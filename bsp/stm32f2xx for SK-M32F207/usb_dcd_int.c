...
uint32_t USBD_OTG_ISR_Handler (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  if (USB_OTG_IsDeviceMode(pdev)) /* ensure that we are in device mode */
  {
    ...
    {
	  if(debug_usbint)
	  KPRINTF("OTG interrupt: coreId=%d, !gintr_status.d32\n", pdev->cfg.coreID);
      return 0;
    }
    ...
    {
	  if(debug_usbint)
	  KPRINTF("OTG interrupt: coreId=%d, outepintr\n", pdev->cfg.coreID);
    ...
    {
	  if(debug_usbint)
	  KPRINTF("OTG interrupt: coreId=%d, inepint\n", pdev->cfg.coreID);
    ...
    if (gintr_status.b.modemismatch)
    {
      ...
      if(debug_usbint)
	  KPRINTF("OTG interrupt: coreId=%d, modemismatch\n", pdev->cfg.coreID);
      /* Clear interrupt */
      gintsts.d32 = 0;
    ...
    {
      if(debug_usbint)
	  KPRINTF("OTG interrupt: coreId=%d, wkupintr\n", pdev->cfg.coreID);
    ...
    {
      if(debug_usbint)
	  KPRINTF("OTG interrupt: coreId=%d, usbsuspend\n", pdev->cfg.coreID);
    ...
    if (gintr_status.b.rxstsqlvl)
    {
	  ...
	  if(debug_usbint)
      KPRINTF("OTG interrupt: coreId=%d, rxstsqlvl\n", pdev->cfg.coreID);
    }
    ...
    if (gintr_status.b.usbreset)
    {
	  ...
	  if(debug_usbint)
      KPRINTF("OTG interrupt: coreId=%d, usbreset\n", pdev->cfg.coreID);
    }
    if (gintr_status.b.enumdone)
    {
      retval |= DCD_HandleEnumDone_ISR(pdev);
	  if(debug_usbint)
	  KPRINTF("OTG interrupt: coreId=%d, enumdone\n", pdev->cfg.coreID);
    }
    
    if (gintr_status.b.incomplisoin)
    {
      if(debug_usbint)
	  KPRINTF("OTG interrupt: coreId=%d, incomplisoin\n", pdev->cfg.coreID);
    ...
    if (gintr_status.b.incomplisoout)
    {
	  ...
	  if(debug_usbint)
	  KPRINTF("OTG interrupt: coreId=%d, incomplisoout\n", pdev->cfg.coreID);
    }    
#ifdef VBUS_SENSING_ENABLED
    if (gintr_status.b.sessreqintr)
    {
      if(debug_usbint)
	  KPRINTF("OTG interrupt: coreId=%d, sessreqintr\n", pdev->cfg.coreID);
    ...
    {
	  if(debug_usbint)
	  KPRINTF("OTG interrupt: coreId=%d, otgintr\n", pdev->cfg.coreID);
...
static uint32_t DCD_HandleInEP_ISR(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  while ( ep_intr )
  {
    if (ep_intr&0x1) /* In ITR */
    {
	  ...
	  if(debug_usbint)
	  KPRINTF("DCD_HandleInEP_ISR() In ITR status: 0x%x\n", diepint.d32);
      if ( diepint.b.xfercompl )
      {
	  ...
	  /* XPH: clear Tx FIFO underrun interrupt */
	  if ( diepint.b.txfifoundrn )
      {
        CLEAR_IN_EP_INTR(epnum, txfifoundrn);
      }
    }
    epnum++;
    ep_intr >>= 1;
