...
extern inline int pit_init(BYTE channel, BYTE mode, WORD tconst)
{
	BYTE v, ch;
	WORD cnt;
...
extern inline int pit_setconstant(BYTE channel, DWORD c)
{
	BYTE v;
	WORD cnt;
	WORD tconst;
...
extern inline WORD pit_read(BYTE channel)
{
    WORD result;
    WORD cnt;
    BYTE ch;
